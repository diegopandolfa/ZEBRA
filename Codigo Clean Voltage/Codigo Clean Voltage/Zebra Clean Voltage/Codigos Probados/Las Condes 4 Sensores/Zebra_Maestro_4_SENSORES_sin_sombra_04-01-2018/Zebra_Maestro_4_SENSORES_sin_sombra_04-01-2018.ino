...
int sensorPromedio(int n)
{
  ...
  for(int i=0;i<n;i++)
  {
    suma=suma*(1-alpha)+analogRead(A0)*alpha;
  }  
  return((int)suma);
}
...
int sensorPromedio_TWO(int n)
{
  ...
  for(int i=0;i<n;i++)
  {
    suma=suma*(1-alpha)+analogRead(A1)*alpha;
  }  
  return((int)suma);
}
...
int sensorPromedio_THREE(int n)
{
  ...
  for(int i=0;i<n;i++)
  {
    suma=suma*(1-alpha)+analogRead(A2)*alpha;
  }  
  return((int)suma);
}
...
int sensorPromedio_FOUR(int n)
{
  ...
  for(int i=0;i<n;i++)
  {
    suma=suma*(1-alpha)+analogRead(A3)*alpha;
  }  
  return((int)suma);
}
...
*/
  if( abs(sensorMax - 294) > 60){
    sensorMax = 294 + 60;  // suelo + 2 desviaciones estandar
  }
  else{
    sensorMax = sensorMax;  
  }
  if( abs(294-sensorMin) > 60){
    sensorMin = 294 - 60; // suelo - 2 desviaciones estandar  
  }
  else{
    sensorMin = sensorMin;  
  }
...
void calibracionTest(){
 ...
 switch (inByte) {
  ...
  if((sensorMax - sensorMin) < 90){ // 3 desviaciones estandar
/*
sensorMin = sensorMin - 50;
sensorMax = sensorMax + 60;
sensorMin_TWO = sensorMin_TWO - 50;
